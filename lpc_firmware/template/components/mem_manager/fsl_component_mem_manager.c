...
void MEM_Trace(shell_handle_t shellHandle)
{
    mem_pool_structure_t *pPool = s_memmanager.pHeadPool;
    block_list_header_t  *pBlock;
    (void)SHELL_Printf(shellHandle, "MEM_Trace debug information.\r\nPools Number: %d   allocationFailures: %d  freeFailures: %d \r\n",
                       s_memmanager.poolNum, s_memmanager.allocationFailures, s_memmanager.allocationFailures,
                       s_memmanager.freeFailures);
    while (NULL != pPool) {
        (void)SHELL_Printf(shellHandle, "POOL ID: %d  blockSize: %d   status:\r\n", pPool->poolId, pPool->blockSize);
        (void)SHELL_Printf(shellHandle,
                           "BlockCount AllocatedBlocks AllocatedBlocksPeak FragmentWaste FragmentWastePeak MinFragmentWaste TotalFragmentWaste\r\n");
        (void)SHELL_Printf(shellHandle,
                           "%7d    %12d    %16d    %10d    %14d    %13d    %15d\r\n",
                           pPool->numBlocks,
                           pPool->allocatedBlocks,
        ...
        for (uint32_t i = 0; i < pPool->numBlocks; i++) {
            ...
            if (pBlock->allocated) {
                (void)SHELL_Printf(shellHandle, "|  %5d  |  0x%08x  |  %6d  |\r\n", i, pBlock->caller, pBlock->allocatedBytes);
            }
        }
        (void)SHELL_Printf(shellHandle, "+---------+--------------+----------+\r\n");
        /* Try next pool*/
